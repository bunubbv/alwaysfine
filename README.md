...
## 제작 동기
2022년 2학기, 학급에서 미세먼지 수치를 매일 측정하고 이를 칠판에 기록하는 역할을 담당했는데 그게 귀찮아서 만들었습니다. 사실 봉사 시간을 받아야 해서 이후에도 계속 칠판에 기록하긴 했지만.. 애플리케이션 개발이 귀찮기도 하고 빠르게 만들고 싶어서 Rainmeter를 사용했는데, 그냥 네이티브 애플리케이션으로 만들 걸 그랬습니다. 아마 귀찮아서 안 만들 것 같습니다.
...
## 이용 범위
AlwaysFine은 GNU Lesser General Public License v2.1 라이선스를 사용하며, 본 프로젝트를 사용하려면 해당 라이선스를 준수해야 합니다. 자세한 내용은 [LICENSE](/LICENSE)를 참조하세요.
 
## 사용 방법
![1](https://github.com/Doiram/AlwaysFine/assets/75381985/bfb1733a-a2cd-4968-af0e-377b28133b80)

스킨을 실행하면 나오는 화면입니다. 미세먼지, 초미세먼지의 수치와 등급이 나오고, 등급에 따라 이모티콘과 배경 색상이 바뀝니다. 측정소는 송현 측정소(제물포고등학교와 가장 가까움)가 기본이며, 등급은 매우 좋음, 좋음, 양호, 보통, 나쁨, 매우 나쁨, 최악으로 총 7단계입니다.
...
설정창입니다. 입력창에 원하는 측정소의 이름(예: 송현)을 입력한 후 엔터를 누르면, 측정소가 변경됩니다. 아무 곳이나 누르면 닫을 수 있습니다. 버전 정보에 마우스를 올리면 자세한 내용을 확인할 수 있습니다. 자동 업데이트가 지원되기 때문에 최신 버전으로 나오는 것이 일반적입니다.
 ...
 * 2023-08-11 5.0.0
     ...
     * 업데이트 구조 변경으로 안정성 향상
        * 개별 다운로드에서 압축 파일 다운로드 방식으로 변경
        * 파일 정상 다운 여부 확인과 해시 값 비교로 무결성 검증 과정 추가
        * 변경된 자동 업데이트 구조의 호환성 문제로 요구 버전 윈도우 10으로 상향
        * 오작동 방지를 위해 업데이트 이후 다시 시작해야 반영되도록 변경
     * 스킨 디자인 및 폰트 변경
